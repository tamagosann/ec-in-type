...
import { ProductsListItem } from 'redux/store/initialState';
...
const ProductEdit = () => {
    ...
    const [name, setName] = useState(''),
          ...
          [priceM, setPriceM] = useState(0),
          [priceL, setPriceL] = useState(0);
    ...
    const submit = useCallback((name: string, description: string, image: string, priceM: number, priceL: number, history: History): void | false => {
        if(name === '' || description === '' || priceM === 0 || priceL === 0 || image === '' ) {
        ...
        const newProduct: ProductsListItem = {
            ...
            <Container maxWidth="sm">
                <Paper variant="outlined" component="div" style={{ padding: 20, marginTop: 40,}} >
                    ...
                    <TextInput fullWidth={true} multiline={false} required={true} label={'Mサイズ価格'}
                        rows={1} value={priceM.toString()} type={'number'} onChange={(e) => inputPriceM(e)}
                    />
                    <TextInput fullWidth={true} multiline={false} required={true} label={'Lサイズ価格'}
                        rows={1} value={priceL.toString()} type={'number'} onChange={(e) => inputPriceL(e)}
                    />
