...
import { InitialState, ProductsList } from "redux/store/initialState";
...
const ProductList = () => {
  ...
  const [showingProducts, setShowingProducts] = useState<ProductsList>([]);
  const [currentPageShowingProducts, setCurrentPageShowingProducts] = useState<ProductsList>([]);
  const [imgaes, setImages] = useState<string[]>([]);
    ...
    <>
      ...
      <Paper className={classes.rootsearch}>
        <InputBase
          ...
          value={keyword}
          onChange={(event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => keywordOnChange(event)}
        />
        <IconButton
          type="button"
