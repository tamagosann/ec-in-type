...
import { Cart, InitialState } from 'redux/store/initialState';
...
const OrderConfirm = () => {
  ...
  const cart: Cart = getProductsInCart(selector);
  ...
  const sum: number = useMemo(() => {
    if(cart.length > 0) {
      ...
      const sumPrice: number = prices.reduce((a, b) => a + b)
      return sumPrice;
    } else {
      return 0
    }
  ...
  // エラーメッセージ表示用
  type ErrMessageObject = {
    [orderProps: string]: string
  }
  const errorMessages: ErrMessageObject = {
    destinationName: '',
    destinationMail: '',        
    destinationZipcode: '',        
    destinationAddress: '',        
  ...
  const destinationTimeChange = useCallback((e: ChangeEvent<{
    name?: string | undefined;
    value: unknown;
}>, child: React.ReactNode): void => {
    setDestinationTime(String(e.target.value));
  }, [setDestinationTime])
  ...
  const additionTime = parseInt(destinationTime) * 60 * 60 * 1000;
  const makeDateTime = selectedDate.getTime() + additionTime
  const selectedDateTime = new Date(makeDateTime) as any
  const nowDateTime = new Date() as any
  ...
  let credit: JSX.Element | undefined
